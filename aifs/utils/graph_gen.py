...
def multi_mesh2(resolutions, xhops=1):
    ...
    ind1 = np.argsort(sp1_rad[:, 1])
    ind2 = np.argsort(sp1_rad[ind1][:, 0])[::-1]
    node_ids = np.arange(sp1_rad.shape[0])

    for ii, coords in enumerate(sp1_rad[ind1][ind2]):
        node_id = node_ids[ind1][ind2][ii]
        G.add_node(node_id, hcoords_rad=sp1_rad[ind1][ind2][ii])

    for ii in node_ids[ind1][ind2]:
    ...
    for resolution in resolutions[:-1]:
        ...
        dist, idx = tree.query(sp2_rad, k=1)
        for ii in range(len(sp2.vertices)):
            for ineighb in x_rings[ii]:
                if ineighb != ii:
                    ...
                    # G.add_edge(idx[ineighb][0], idx[ii][0], \
                    # edge_attr=(haversine_distances([loc_neigh, loc_dst])[0][1], *direction))
                    G.add_edge(idx[ineighb][0], idx[ii][0], weight=haversine_distances([loc_neigh, loc_dst])[0][1])

    return G, sp1_rad[ind1][ind2]
