...
class AIFSGradientChecker:
    ...
    def log_information(self) -> None:
        ...
        total_number_of_model_instances = (
            self.config.hardware.num_nodes * self.config.hardware.num_gpus_per_node / self.config.hardware.num_gpus_per_node
        )
        LOGGER.debug(
    ...
    def strategy(self) -> Any:
        return DDPGroupStrategy(
            self.config.hardware.group_size,
    ...
    def check_gradients(self) -> None:
        ...
        for batch_idx, batch in enumerate(dl_train):
            ...
            train_loss, _, _, _ = self.model._step(batch[0], x_ens_ic)
            train_loss.backward()
            LOGGER.debug("Ran backward ...")

            gnorm_sum = self._calculate_grad_norms()
            LOGGER.debug(
                "GlobalRank %d: batch_size=%d, gpus_per_node=%d, ens_per_device=%d - sum(||grad(p)||)=%.9e",
