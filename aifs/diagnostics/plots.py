...
def plot_predicted_ensemble(
    ...
    Args:
        ...
        y_true: array of shape (bs, latlon, nvar) -- "truth" (target)
        y_pred: array of shape (bs, latlon, nvar, nens) -- predicted ensemble
    Returns:
        The figure object handle.
    """
    assert 0 <= sample_index <= y_true.shape[0], f"Invalid sample_index {sample_index}! Must range from 0 to {y_true.shape[0]}"

    y_true = y_true[sample_index, ...].squeeze()
    y_pred = y_pred[sample_index, ...].squeeze()
    ...
    for plot_idx, (variable_idx, variable_name) in enumerate(parameters.items()):
        ...
        yp = y_pred[..., variable_idx].squeeze()
        if n_plots_x > 1:
            plot_ensemble(fig, ax[plot_idx, :], pc, latlons, yt, yp, variable_name)
        else:
            plot_ensemble(fig, ax, pc, latlons, yt, yp, variable_name)
    return fig
...
def plot_kcrps(parameters: Dict[str, int], latlons: np.ndarray, pkcrps: np.ndarray) -> Figure:
    ...
    Args:
        parameters: variable/index mapping, from config
        latlons: lat/lon coordinates array, shape (lat*lon, 2)
        pkcrps: array of pointwise kcrps values, shape (nvar, latlon)
    """
    assert latlons.shape[0] == pkcrps.shape[1], f"Oops - shape mismatch! {latlons.shape[0]} != {pkcrps.shape[1]}"

    fig, ax = plt.subplots(1, len(parameters), figsize=(len(parameters) * 4, 3), subplot_kw={"projection": ccrs.PlateCarree()})
    lat, lon = latlons[:, 0], latlons[:, 1]

    for plot_idx, (variable_idx, variable_name) in enumerate(parameters.items()):
        pkcrps_ = pkcrps[variable_idx, :].squeeze()
        scatter_plot(fig, ax[plot_idx], ccrs.PlateCarree(), lat, lon, pkcrps_, title=f"{variable_name} kCRPS")
    return fig
...
def plot_ensemble(
    ...
    """Use this when plotting ensembles, where each member is defined on "flat"
    grids."""
