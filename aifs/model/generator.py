...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        self.activation = config.model.generator.activation
        self.noise_channels = config.model.generator.noise_channels

        LOGGER.debug("in_channels + aux_in_channels == %d", self.in_channels + self.aux_in_channels)
        LOGGER.debug("num_aux_features = %d", config.data.num_aux_features)
        LOGGER.debug("Noise channels: %d", self.noise_channels)
        ...
        self.era_trainable_size = config.model.generator.trainable_parameters.era
        self.era_trainable = (
            nn.Parameter(torch.zeros(self._era_size, self.era_trainable_size)) if self.era_trainable_size > 0 else None
        )

        self.h_trainable_size = config.model.generator.trainable_parameters.hidden
        self.h_trainable = nn.Parameter(torch.zeros(self._h_size, self.h_trainable_size)) if self.h_trainable_size > 0 else None

        self.e2h_trainable_size = config.model.generator.trainable_parameters.era2hidden
        self.e2h_trainable = (
            nn.Parameter(
                torch.zeros(
        ...
        self.h2e_trainable_size = config.model.generator.trainable_parameters.hidden2era
        self.h2e_trainable = (
            nn.Parameter(
                torch.zeros(
        ...
        self.h2h_trainable_size = config.model.generator.trainable_parameters.hidden2hidden
        self.h2h_trainable = (
            nn.Parameter(
                torch.zeros(
        ...
        encoder_out_channels = config.model.generator.num_channels
        mlp_extra_layers = config.model.generator.mlp.extra_layers
        ...
        self.forward_mapper = MessagePassingMapper(
            ...
            hidden_layers=config.model.generator.encoder.num_layers,
        ...
        self.h_processor = MessagePassingProcessor(
            ...
            hidden_layers=config.model.generator.hidden.num_layers,
            mlp_extra_layers=mlp_extra_layers,
        ...
        self.backward_mapper = MessagePassingMapper(
            ...
            hidden_dim=config.model.generator.num_channels,
            hidden_layers=config.model.generator.decoder.num_layers,
            mlp_extra_layers=mlp_extra_layers,
    ...
    def forward(self, z: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            z: noise tensor, shape = (*x_latent.shape[:-1], noise_channels), same type as x_latent
            x: input tensor, shape (bs, mstep, latlon, fc_dim + aux_dim)
        Returns:
            Model prediction, shape (bs, latlon, fc_dim)
        """
        bs = x.shape[0]
        ...
        # skip connection (H -> H), leaves out the last self.noise_channels
        # workaround needed b/c the h_processor inputs and outputs have equal channel size
        # TODO [Mihai]: if you come up with a cleaner way to inject the noise to the latents, I'm all ears
        # (probably minor re-factoring of the processor logic is needed to allow this)
