        ...
        self.proc = nn.ModuleList(
            [
                MessagePassingProcessorChunk(
                    hidden_dim, edge_dim, hidden_layers=chunk_size, mlp_extra_layers=mlp_extra_layers, activation=activation, ptype=ptype
                )
...
class MessagePassingProcessorChunk(nn.Module):
    ...
    def __init__(
        ...
        self.proc = nn.ModuleList(
            [
                # MessagePassingBlock(hidden_dim, hidden_dim, hidden_dim, mlp_extra_layers=mlp_extra_layers, activation=activation)
                # GATv2Block(hidden_dim, 4*hidden_dim, hidden_dim, edge_dim, mlp_extra_layers=mlp_extra_layers, activation=activation, ptype=ptype)
                TransformerBlock(hidden_dim, 4*hidden_dim, hidden_dim, edge_dim, mlp_extra_layers=mlp_extra_layers, activation=activation, ptype=ptype)
                for _ in range(self.hidden_layers)
            ]
        )
...
class GATv2Block(nn.Module):
    def __init__(
        ...
        activation: str = "SiLU",
        dropout: float = 0.0,
        fill_value: Union[float, Tensor, str] = 0.0,
        ptype: str = "proc",
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        ...
        self.conv = tgnn.GATv2Conv(
            ...
            concat=True,
            edge_dim=edge_dim, # todo: enable edge_dim=3
...
#     """Message passing part of transformer block."""
